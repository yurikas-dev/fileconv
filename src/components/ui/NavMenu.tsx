...
export function NavMenu({ locale }: Props) {
  ...
  const navItems: NavItem[] = [
    {
      ...
      subItems: [
        { label: 'HEIC → JPG / PNG', href: `${prefix}/tools/converter/heic-to-jpg` },
        { label: 'JPG → PNG / WebP', href: `${prefix}/tools/converter/jpg-converter` },
        { label: 'PNG → JPG / WebP', href: `${prefix}/tools/converter/png-converter` },
        { label: 'WebP → JPG / PNG', href: `${prefix}/tools/converter/webp-converter` },
      ],
    },
    {
      kind: 'dropdown',
  ...
  return (
    <div ref={menuRef} className="flex items-center">
      ...
      {/* ── Mobile panel ── */}
      {mobileOpen && (
        <div className="fixed top-14 left-0 right-0 bottom-0 z-40 bg-black/20" onClick={() => setMobileOpen(false)}>
          <div
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center px-5 py-3.5 text-sm border-b border-gray-100 transition-colors ${
                      isActive ? 'text-brand-600 font-semibold bg-brand-50' : 'text-gray-700 hover:bg-gray-50'
                    }`}
                  >
                    {item.label}
                  </Link>
                ...
                <div key={item.label} className="border-b border-gray-100">
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-5 py-3.5 text-sm transition-colors ${
                      isActive ? 'text-brand-600 font-semibold' : 'text-gray-700 hover:bg-gray-50'
                    }`}
                  >
                    {item.label}
                    <ChevronRight
