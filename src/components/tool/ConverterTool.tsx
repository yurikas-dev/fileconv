...
export function ConverterTool() {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm mb-12">
      {/* 形式選択 */}
      <div className="mb-5">
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2.5">変換形式</p>
      ...
      <div className={`mb-5 transition-opacity ${format === 'png' ? 'opacity-40 pointer-events-none' : ''}`}>
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2.5">
          画質{format === 'png' && <span className="normal-case font-normal ml-1">（PNGは可逆圧縮のため固定）</span>}
        </p>
        <div className="flex gap-2">
          {QUALITIES.map(q => (
            <button key={q.label} onClick={() => setQuality(q)}
      ...
      {/* EXIF削除 */}
      <div className={`mb-5 transition-opacity ${format === 'png' ? 'opacity-40 pointer-events-none' : ''}`}>
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2.5">プライバシー設定</p>
        <button
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-sm font-medium text-gray-800">EXIF情報を削除する</span>
            </div>
            <p className="text-xs text-gray-500 mt-0.5">撮影場所（GPS）・日時・カメラ情報などの個人情報を除去します</p>
          </div>
          {stripExifOn && format === 'jpg' && (
        ...
        {format === 'png' && (
          <p className="text-xs text-gray-400 mt-1.5 pl-1">PNGはEXIF情報を含まないため、この設定は適用されません</p>
        )}
      </div>
      ...
      >
        ...
        <p className="text-sm font-semibold text-gray-600 mb-1">HEICファイルをここにドロップ</p>
        <p className="text-xs text-gray-400">または <span className="text-brand-600 font-medium">クリックして選択</span>（複数可）</p>
      </div>
      ...
      {files.length > 0 && (
        <div className="flex flex-col gap-2 mb-4">
          {files.map((item, i) => (
            <FileRow key={`${item.file.name}-${i}`} item={item} format={format} onRemove={() => removeFile(i)} />
          ))}
        </div>
      )}
      ...
      {allDone && (
        <div className="flex items-center gap-3 bg-teal-50 border border-teal-200 rounded-xl px-4 py-3 mb-4 text-sm font-medium text-teal-700">
          ...
          すべて変換完了！各ファイルのダウンロードボタンからご利用ください。
        </div>
      )}
      ...
      >
        {files.some(f => f.status === 'converting') ? '変換中...' : '変換スタート'}
      </button>
      ...
      {files.length > 0 && (
        <div className="grid grid-cols-3 gap-2 mt-4 pt-4 border-t border-gray-100">
          {[
            { label: '選択ファイル', value: files.length },
            { label: '変換完了',     value: doneCount },
            { label: '合計サイズ',   value: `${(totalSize / 1024 / 1024).toFixed(1)}MB` },
          ].map(s => (
...
function FileRow({ item, format, onRemove }: { item: FileItem; format: Format; onRemove: () => void }) {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2.5 bg-gray-50 rounded-xl border border-gray-100">
      ...
      {item.status === 'waiting'    && <span className="text-xs text-gray-400 bg-gray-100 px-2 py-1 rounded-full">待機中</span>}
      ...
      {item.status === 'error' && <span className="text-xs text-red-600 bg-red-50 px-2 py-1 rounded-full">エラー</span>}
      {item.status === 'waiting' && (
